...
#include "Engine/Source/Runtime/Core/Public/Logging/LogTrace.h"
...
#define UE_LOG_EXPAND_IS_FATAL_SetColor(   ActiveBlock, InactiveBlock) InactiveBlock
	...
	**/
	...
	#define DECLARE_LOG_CATEGORY_OFFSET(CategoryName, DefaultVerbosity, CompileTimeVerbosity) \
		struct FLogCategory##CategoryName : public FLogCategory<ELogVerbosity::DefaultVerbosity, ELogVerbosity::CompileTimeVerbosity> \
		{ \
			FORCEINLINE FLogCategory##CategoryName() : FLogCategory(TEXT(#CategoryName)) {} \
		};

	#define UE_LOG_CATEGORY_AT(CategoryName, Offset) (*reinterpret_cast<FLogCategory##CategoryName*>(InSDKUtils::GetImageBase() + (Offset)))
